...
func TestBackgroundShell_Integration(t *testing.T) {
	...
	bgShell, err := bgManager.Start(ctx, workingDir, nil, "echo 'hello background' && echo 'done'", "", nil)
...
func TestBackgroundShell_Kill(t *testing.T) {
	...
	bgManager := shell.GetBackgroundShellManager()
	bgShell, err := bgManager.Start(ctx, workingDir, nil, "sleep 100", "", nil)
	require.NoError(t, err)
...
func TestBackgroundShell_MultipleOutputCalls(t *testing.T) {
	...
	// Start a background shell
	bgManager := shell.GetBackgroundShellManager()
	bgShell, err := bgManager.Start(ctx, workingDir, nil, "echo 'step 1' && echo 'step 2' && echo 'step 3'", "", nil)
	require.NoError(t, err)
...
func TestBackgroundShell_EmptyOutput(t *testing.T) {
	...
	bgManager := shell.GetBackgroundShellManager()
	bgShell, err := bgManager.Start(ctx, workingDir, nil, "sleep 0.1", "", nil)
	require.NoError(t, err)
	defer bgManager.Kill(bgShell.ID)
...
func TestBackgroundShell_ExitCode(t *testing.T) {
	...
	bgManager := shell.GetBackgroundShellManager()
	bgShell, err := bgManager.Start(ctx, workingDir, nil, "echo 'failing' && exit 42", "", nil)
	require.NoError(t, err)
	defer bgManager.Kill(bgShell.ID)
...
func TestBackgroundShell_WithBlockFuncs(t *testing.T) {
	...
	bgManager := shell.GetBackgroundShellManager()
	bgShell, err := bgManager.Start(ctx, workingDir, blockFuncs, "curl example.com", "", nil)
	require.NoError(t, err)
	defer bgManager.Kill(bgShell.ID)
...
func TestBackgroundShell_StdoutAndStderr(t *testing.T) {
	...
	bgManager := shell.GetBackgroundShellManager()
	bgShell, err := bgManager.Start(ctx, workingDir, nil, "echo 'stdout message' && echo 'stderr message' >&2", "", nil)
	require.NoError(t, err)
	defer bgManager.Kill(bgShell.ID)
...
func TestBackgroundShell_ConcurrentAccess(t *testing.T) {
	...
	// Start a background shell
	bgManager := shell.GetBackgroundShellManager()
	bgShell, err := bgManager.Start(ctx, workingDir, nil, "for i in 1 2 3 4 5; do echo \"line $i\"; sleep 0.05; done", "", nil)
	require.NoError(t, err)
	defer bgManager.Kill(bgShell.ID)
...
func TestBackgroundShell_List(t *testing.T) {
	...
	for i := range 3 {
		bgShell, err := bgManager.Start(ctx, workingDir, nil, "sleep 1", "", nil)
		require.NoError(t, err)
		shells[i] = bgShell
	}
...
func TestBackgroundShell_AutoBackground(t *testing.T) {
	...
	t.Run("quick command completes synchronously", func(t *testing.T) {
		t.Parallel()
		bgManager := shell.GetBackgroundShellManager()
		bgShell, err := bgManager.Start(ctx, workingDir, nil, "echo 'quick'", "", nil)
		require.NoError(t, err)
	...
	t.Run("long command stays in background", func(t *testing.T) {
		t.Parallel()
		bgManager := shell.GetBackgroundShellManager()
		bgShell, err := bgManager.Start(ctx, workingDir, nil, "sleep 20 && echo '20 seconds completed'", "", nil)
		require.NoError(t, err)
		defer bgManager.Kill(bgShell.ID)
