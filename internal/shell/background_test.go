...
func TestBackgroundShellManager_Start(t *testing.T) {
	...
	bgShell, err := manager.Start(ctx, workingDir, nil, "echo 'hello world'", "", nil)
	if err != nil {
...
func TestBackgroundShellManager_Get(t *testing.T) {
	...
	bgShell, err := manager.Start(ctx, workingDir, nil, "echo 'test'", "", nil)
	if err != nil {
		t.Fatalf("failed to start background shell: %v", err)
	}
...
func TestBackgroundShellManager_Kill(t *testing.T) {
	...
	bgShell, err := manager.Start(ctx, workingDir, nil, "sleep 10", "", nil)
	if err != nil {
		t.Fatalf("failed to start background shell: %v", err)
	}
...
func TestBackgroundShell_IsDone(t *testing.T) {
	...
	bgShell, err := manager.Start(ctx, workingDir, nil, "echo 'quick'", "", nil)
	if err != nil {
		t.Fatalf("failed to start background shell: %v", err)
	}
...
func TestBackgroundShell_WithBlockFuncs(t *testing.T) {
	...
	bgShell, err := manager.Start(ctx, workingDir, blockFuncs, "curl example.com", "", nil)
	if err != nil {
		t.Fatalf("failed to start background shell: %v", err)
	}
...
func TestBackgroundShellManager_List(t *testing.T) {
	...
	// Start two shells
	bgShell1, err := manager.Start(ctx, workingDir, nil, "sleep 1", "", nil)
	if err != nil {
		t.Fatalf("failed to start first background shell: %v", err)
	}

	bgShell2, err := manager.Start(ctx, workingDir, nil, "sleep 1", "", nil)
	if err != nil {
...
func TestBackgroundShellManager_KillAll(t *testing.T) {
	...
	shell1, err := manager.Start(ctx, workingDir, nil, "sleep 10", "", nil)
	if err != nil {
		t.Fatalf("failed to start shell 1: %v", err)
	}

	shell2, err := manager.Start(ctx, workingDir, nil, "sleep 10", "", nil)
	if err != nil {
		t.Fatalf("failed to start shell 2: %v", err)
	}

	shell3, err := manager.Start(ctx, workingDir, nil, "sleep 10", "", nil)
	if err != nil {
...
func TestBackgroundShellManager_KillAll_Timeout(t *testing.T) {
	...
	_, err := manager.Start(t.Context(), workingDir, nil, "trap '' TERM INT; sleep 60", "", nil)
