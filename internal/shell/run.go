...
func execHandlerOption(cwd string, blockFuncs []BlockFunc, sandbox ...*SandboxConfig) interp.RunnerOption {
	base := processGroupExecHandler(defaultKillTimeout)
	handler := base
	for _, mw := range slices.Backward(standardHandlers(cwd, blockFuncs, sandbox...)) {
		handler = mw(handler)
	}
...
func standardHandlers(cwd string, blockFuncs []BlockFunc, sandbox ...*SandboxConfig) []func(next interp.ExecHandlerFunc) interp.ExecHandlerFunc {
	...
	if len(sandbox) > 0 && sandbox[0] != nil {
		handlers = append(handlers, sandboxHandler(cwd, sandbox[0]))
	}
	if useGoCoreUtils {
